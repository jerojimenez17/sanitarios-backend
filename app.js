...
app.get("/api/productos/cerrajeria",(req,res)=>{
    const products = readExcelFile(6, [3, 1, 0, -1, 2], "./excel-files/cerrajeria.xlsx");
    res.send(products.map(product =>
        ({...product, price: product.price *1.11 * 1.5})));
});
app.get("/api/productos/paulo",(req,res)=>{
    const products = readExcelFile(1, [0, 1, -1, -1, 2], "./excel-files/paulo1.xlsx");
    res.send(products.map(product =>
        ({...product, price: product.price *1.11 * 1.5})));
});
...
app.get("/api/productos/bethular",(req,res)=>{
    const products = readExcelFile(6, [1, 2, -1, -1, 3], "./excel-files/bethular.xlsx");
    res.send(products.map(product =>
        ({...product, price: product.price* 1.21* 1.5})));
});
...
app.get("/api/productos/foxs",(req,res)=>{
    const products = readExcelFile(2, [0, 1, -1, -1, 2], "./excel-files/foxs.xlsx");
    res.send(products.map(product =>
        ({...product, price: (parseFloat(product.price) - parseFloat(product.price)*0.2) * 1.50}))); 
});
